import torch
import torch.nn as nn
from Embeddings import MSA_emb, Extra_emb, Templ_emb, Recycling
from Track_module import IterativeFeatureExtractor
from AuxiliaryPredictor import DistanceNetwork, MaskedTokenNetwork 
from constant import INIT_CRDS
from .kinematics import xyz_to_t2d

class RoseTTAFoldModule(nn.Module):
    def __init__(self, n_module_2track=4, n_module_3track=4,\
                 d_msa=256, d_msa_full=64, d_pair=128, d_templ=64,
                 n_head_msa=8, n_head_pair=4, n_head_templ=4,
                 d_hidden=32, d_hidden_templ=64,
                 p_drop=0.15,
                 SE3_param={'l0_in_features':32, 'l0_out_features':16, 'num_edge_features':32}):
        super(RoseTTAFoldModule, self).__init__()
        #
        self.latent_emb = MSA_emb(d_msa=d_msa, d_pair=d_pair, p_drop=p_drop)
        self.full_emb = Extra_emb(d_msa=d_msa_full, d_init=23, p_drop=p_drop)
        self.templ_emb = Templ_emb(d_pair=d_pair, d_templ=d_templ, n_head=n_head_templ,
                                   d_hidden=d_hidden_templ, p_drop=0.25)
        self.recycle = Recycling(d_msa=d_msa, d_pair=d_pair)
        #
        self.extractor = IterativeFeatureExtractor(n_module_2track=n_module_2track,
                                                   n_module_3track=n_module_3track,
                                                   d_msa=d_msa, d_msa_full=d_msa_full,
                                                   d_pair=d_pair, d_hidden=d_hidden,
                                                   n_head_msa=n_head_msa, n_head_pair=n_head_pair,
                                                   SE3_param=SE3_param,
                                                   p_drop=p_drop)
        ##
        self.c6d_pred = DistanceNetwork(d_pair, p_drop=p_drop)
        self.aa_pred = MaskedTokenNetwork(d_msa, p_drop=p_drop)

    def forward(self, msa, seq=None, idx=None, t1d=None, t2d=None, msa_one_hot=None,
                msa_prev=None, pair_prev=None, xyz_prev=None,
                return_raw=False, return_full=False,
                use_transf_checkpoint=False):

        B, N, L = msa.shape

        if seq is None:
            seq = msa[:,0]
        if idx is None:
            idx = torch.arange(L, device=msa.device).unsqueeze(0).expand(B,-1)

        if msa_one_hot is not None:
            if msa_one_hot.shape[-1] < 22:
                pad_size = 22 - msa_one_hot.shape[-1]
                pad_row = torch.zeros(1,1,L,pad_size).to(msa_one_hot.device)
                msa_one_hot = torch.cat([msa_one_hot, pad_row],-1) # pad to 22 tokens
            seq1hot = msa_one_hot[:,0]
        else:
            msa_one_hot = torch.nn.functional.one_hot(msa, num_classes=22).float()
            seq1hot = msa_one_hot[:,0]

        msa_feat = torch.cat([msa_one_hot, msa_one_hot, torch.zeros(B,1,L,2).to(msa_one_hot.device)], dim=3)
        extra_feat = torch.cat([msa_one_hot, torch.zeros(B,1,L,1).to(msa_one_hot.device)], dim=3)

        # Get embeddings - msa_latent and msa_full are the same
        msa_latent, pair = self.latent_emb(msa_feat, seq, idx, seq1hot=seq1hot)
        msa_full = self.full_emb(extra_feat, seq, idx, seq1hot=seq1hot)
        #
        if msa_prev is None:
            msa_prev = torch.zeros_like(msa_latent[:,0])
            pair_prev = torch.zeros_like(pair)

        if xyz_prev is None:
            xyz_prev = INIT_CRDS.to(msa_latent.device).reshape(1,1,3,3).expand(B,L,-1,-1)
            #xyz_prev = torch.zeros((B, L, 3, 3), device=msa_latent.device).float()

        msa_recycle, pair_recycle = self.recycle(msa_prev, pair_prev, xyz_prev)
        msa_latent[:,0] = msa_latent[:,0] + msa_recycle.reshape(B,L,-1)
        pair = pair + pair_recycle
        #
        # add template embedding
        if t1d is None:
            t1d = torch.nn.functional.one_hot(torch.full((B,1,L), 20).long(), num_classes=21).float()
            t1d = torch.cat((t1d, torch.zeros((B,1,L,1)).float()), -1).to(msa.device)
            xyz_t = torch.full((B,1,L,3,3), float('nan')).float()
            t2d = xyz_to_t2d(xyz_t).to(msa.device)

            #t1d = torch.zeros((B, 1, L, 22), device=msa.device).float()
            #t2d = torch.zeros((B, 1, L, L, 44), device=msa.device).float()
        pair = self.templ_emb(t1d, t2d, pair, use_checkpoint=use_transf_checkpoint)
        
        msa, pair, xyz, lddt = self.extractor(msa_latent, msa_full, pair, idx,
                                              use_checkpoint=use_transf_checkpoint)
        if return_full:
            return msa[:,0], pair, xyz.reshape(-1, B, L, 3, 3)[-1], \
                   logits, logits_aa, xyz.reshape(-1, B, L, 3, 3), lddt.view(B, L)

        if return_raw:
            return msa[:,0], pair, xyz.reshape(-1, B, L, 3, 3)[-1]
        #
        logits = self.c6d_pred(pair)
        logits_aa = self.aa_pred(msa)
        #return logits, logits_aa, xyz.reshape(-1, B, L, 3, 3), lddt.view(B, L)

        prob_s = list()
        for l in logits:
            prob_s.append(nn.Softmax(dim=1)(l)) # (B, C, L, L)
        prob_s = torch.cat(prob_s, dim=1).permute(0,2,3,1) # (B,L,L,C)

        out = dict(
            dist = logits[0],
            omega = logits[1],
            theta = logits[2],
            phi = logits[3],
            xyz = xyz.reshape(-1,B,L,3,3)[-1],
            lddt = lddt.view(B,L),
            prob_s = prob_s,
            logits_aa = logits_aa,
            pair = pair,
            msa = msa
        )

        return out
