...
class DropHead(nn.Module):
    # Drop entire attention head during training
    # reference: scheduled drophead
    ...
    def forward(self, x):
        # x: input attention map
        if not self.training: # no drophead during evaluation mode
            return x
        B, h = x.shape[:2] 
        mask = self.sampler.sample((B, h)).to(x.device)
        mask = mask.view(B, h, 1, 1)
        ones = mask.sum(dim=1, keepdim=True) # number of active heads (B, 1, 1, 1)
        ones = ones.expand(-1, h, -1, -1)
        mask = torch.where(ones > 0.0, mask, torch.ones((B, h, 1, 1), device=x.device))
        ones = mask.sum(dim=1, keepdim=True) # number of active heads (B, 1, 1, 1)
        #
        x = mask * x * h / ones # (h/ones are multiplied to normalize output)
...
class MultiheadAttention(nn.Module):
    def __init__(self, d_model, heads, k_dim=None, v_dim=None, dropout=0.1):
        ...
        self.dropout = DropHead(0.05)
        self.max_tokens = 5000000

    def calc_attention(self, q, k, v, return_att=False):
        batch, _, L1 = q.shape[:3]
        #
    ...
    def _batched_forward(self, q, k, v):
        b, h, n1 = q.shape[:3]
        n2 = k.shape[2]
        out = torch.empty((b,n1,self.d_model), dtype=q.dtype, device=q.device)
        shift = self.max_tokens // (n1*n2)
        #
        for i_b in range(0, b, shift):
            start = i_b
            end = min(i_b+shift, b)
            out[start:end] = self.calc_attention(q[start:end], k[start:end], v[start:end])
        return out
    ...
    def forward(self, query, key, value, return_att=False):
        ...
        if not torch.is_grad_enabled() and (not return_att) and (batch*L1*L2 > self.max_tokens):
            out = self._batched_forward(q, k, v)
        else:
            if return_att:
                out, attention = self.calc_attention(q, k, v, return_att=return_att)
            else:
                out = self.calc_attention(q, k, v, return_att=False)
...
class TiedMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, return_att=False):
        ...
        out = torch.einsum("bhij,bnhjk->bnhik", attention, v) # (B, N, H, L, d_k)
...
class MaskedDirectMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        attention = attention.masked_fill(mask, torch.finfo(q.dtype).min)
        attention = F.softmax(attention, dim=-1) # (B, h, L1, L2)
        attention = self.dropout(attention)
        #
...
class AxialEncoderLayer(nn.Module):
    ...
    def forward(self, src, return_att=False):
        ...
        if self.use_tied_row or self.use_soft_row:
            if return_att:
                src2, att = self.attn_L(src2, src2, src2, return_att=return_att) # Tied attention over L
            else:
