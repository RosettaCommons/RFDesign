...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, p_drop=0.0, max_len=5000):
...
class PositionalEncoding2D(nn.Module):
    def __init__(self, d_model, p_drop=0.0):
...
class MSA_emb(nn.Module):
    def __init__(self, d_model=64, d_msa=21, p_drop=0.0, max_len=5000):
...
class Templ_emb(nn.Module):
    def __init__(self, d_t1d=3, d_t2d=10, d_templ=64, n_att_head=4, r_ff=4,
                 performer_opts=None, p_drop=0.0, max_len=5000):
...
class Pair_emb_w_templ(nn.Module):
    def __init__(self, d_model=128, d_seq=21, d_templ=64, p_drop=0.0):
    ...
    def forward(self, seq, idx, templ, one_hot_seq=None):
        # input:
        ...
        if one_hot_seq is not None:
            seq = one_hot_seq @ self.emb.weight
        else:
...
class Pair_emb_wo_templ(nn.Module):
    ...
    def __init__(self, d_model=128, d_seq=21, p_drop=0.0):
        ...
        self.pos = PositionalEncoding2D(d_model, p_drop=p_drop)
    def forward(self, seq, idx, one_hot_seq=None):
        # input:
        #   seq: target sequence (B, L, 20)
        B = seq.shape[0]
        L = seq.shape[1]
