...
class Refine_module(nn.Module):
    ...
    def _iterative_forward(self, node, edge, xyz, state, seq1hot, idx, eps=1e-4):
        best_xyz = None
    ...
    def forward(self, node, edge, seq1hot, idx, use_transf_checkpoint=False, eps=1e-4):
        ...
        #if not torch.is_grad_enabled(): # inference mode -- run iteratively!
        #    xyz, lddt = self._iterative_forward(node, edge, xyz, state, seq1hot, idx, eps=eps)
        #else: # gradients are required -- iteration might not work
        for i_m in range(self.n_module):
