...
class CoevolExtractor(nn.Module):
    def __init__(self, n_feat_proj, n_feat_out, p_drop=0.0):
...
class MSA2Pair(nn.Module):
    def __init__(self, n_feat=64, n_feat_out=128, n_feat_proj=32,
                 n_resblock=1, p_drop=0.0, n_att_head=8):
    ...
    def forward(self, msa, pair_orig, att, use_chk=False):
...
class MSA2MSA(nn.Module):
    def __init__(self, n_layer=1, n_att_head=8, n_feat=256, r_ff=4, p_drop=0.0,
    ...
    def forward(self, x, use_chk=False):
        # Input: MSA embeddings (B, N, L, K)
        # Output: updated MSA embeddings (B, N, L, K)
        B, N, L, _ = x.shape
        # attention along L
        x, att = self.encoder_1(x, return_att=True, use_chk=use_chk)
        # attention along N
        x = x.permute(0,2,1,3).contiguous()
        x = self.encoder_2(x, use_chk=use_chk)
        x = x.permute(0,2,1,3).contiguous()
        return x, att

class Pair2MSA(nn.Module):
    def __init__(self, n_layer=1, n_att_head=4, n_feat_in=128, n_feat_out=256, r_ff=4, p_drop=0.0):
    ...
    def forward(self, pair, msa, use_chk=False):
        out = self.encoder(pair, msa, use_chk=use_chk) # (B, N, L, K)
        return out

class Pair2Pair(nn.Module):
    def __init__(self, n_layer=1, n_att_head=8, n_feat=128, r_ff=4, p_drop=0.0,
    ...
    def forward(self, x, use_chk=False):
        return self.encoder(x, use_chk=use_chk)
...
class Str2MSA(nn.Module):
    def __init__(self, d_msa=64, d_state=32, inner_dim=32, r_ff=4,
                 distbin=[8.0, 12.0, 16.0, 20.0], p_drop=0.0):
    ...
    def forward(self, msa, xyz, state, use_chk=False):
        ...
        msa2 = self.norm1(msa)
        if use_chk:
            msa2 = checkpoint.checkpoint(create_custom_forward(self.attn), state, state, msa2, mask_s)
        else:
...
class IterBlock(nn.Module):
    def __init__(self, n_layer=1, d_msa=64, d_pair=128, n_head_msa=4, n_head_pair=8, r_ff=4,
                 n_resblock=1, p_drop=0.0, performer_L_opts=None, performer_N_opts=None):
    ...
    def forward(self, msa, pair, use_transf_checkpoint=False):
        ...
        msa, att = self.msa2msa(msa, use_chk=use_transf_checkpoint)
        
        # 2. update pair features using given MSA
        pair = self.msa2pair(msa, pair, att, use_chk=use_transf_checkpoint)

        # 3. process pair features
        pair = self.pair2pair(pair, use_chk=use_transf_checkpoint)
        
        # 4. update MSA features using updated pair features
        msa = self.pair2msa(pair, msa, use_chk=use_transf_checkpoint)
...
class IterBlock_w_Str(nn.Module):
    ...
    def forward(self, msa, pair, xyz, seq1hot, idx, top_k=64,
        ...
        if use_transf_checkpoint:
            xyz, state = checkpoint.checkpoint(create_custom_forward(self.str2str, top_k=top_k),
                    msa.float(), pair.float(), xyz.float(), seq1hot, idx)
            msa = checkpoint.checkpoint(create_custom_forward(self.str2msa), msa, xyz, state)
        else:
...
class FinalBlock(nn.Module):
    ...
    def forward(self, msa, pair, xyz, seq1hot, idx, use_transf_checkpoint=False):
        # input:
        #   msa: initial MSA embeddings (N, L, d_msa)
        #   pair: initial residue pair embeddings (L, L, d_pair)
        ...
        if use_transf_checkpoint:
            xyz, state = checkpoint.checkpoint(create_custom_forward(self.str2str, top_k=32),
                    msa.float(), pair.float(), xyz.float(), seq1hot, idx)
        else:
...
class IterativeFeatureExtractor(nn.Module):
    def __init__(self, n_module=4, n_module_str=4, n_layer=4, d_msa=256, d_pair=128, d_hidden=64,
        ...
        self.final = FinalBlock(n_layer=n_layer, d_msa=d_msa, d_pair=d_pair,
                               ...
                               SE3_param=SE3_param).to(device1)
    
    def forward(self, msa, pair, seq1hot, idx, use_transf_checkpoint=False):
        # input:
        #   msa: initial MSA embeddings (N, L, d_msa)
        #   pair: initial residue pair embeddings (L, L, d_pair)
        
        pair_s = list()
        pair = self.initial(pair, use_chk=use_transf_checkpoint)
        if self.n_module > 0:
        ...
        if self.n_module_str > 0:
            for i_m in range(self.n_module_str):
                msa, pair, xyz = self.iter_block_2[i_m](msa, pair, xyz, seq1hot, idx,
                        top_k=top_ks[i_m], use_transf_checkpoint=use_transf_checkpoint)

        msa = msa.to(self.device1)
        pair = pair.to(self.device1)
        xyz = xyz.to(self.device1)
