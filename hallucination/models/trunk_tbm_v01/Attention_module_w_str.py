...
class IterBlock_w_Str(nn.Module):
    ...
    def forward(self, msa, pair, xyz, seq1hot, idx, top_k=64, use_transf_checkpoint=False):
        # input:
        #   msa: initial MSA embeddings (N, L, d_msa)
        #   pair: initial residue pair embeddings (L, L, d_pair)
        if use_transf_checkpoint:
        ...
        if use_transf_checkpoint:
            xyz, state = checkpoint.checkpoint(create_custom_forward(self.str2str, top_k=top_k), msa.float(), pair.float(), xyz.float(), seq1hot, idx)
...
class FinalBlock(nn.Module):
    ...
    def forward(self, msa, pair, xyz, seq1hot, idx, use_transf_checkpoint=False):
        ...
        if use_transf_checkpoint:
            xyz, state = checkpoint.checkpoint(create_custom_forward(self.str2str, top_k=32), msa.float(), pair.float(), xyz.float(), seq1hot, idx)
            Transformer.USE_CHECKPOINT = False
        else:
...
class IterativeFeatureExtractor(nn.Module):
    ...
    def forward(self, msa, pair, seq1hot, idx, use_transf_checkpoint=False):
        ...
        xyz_s = list()
        xyz = self.init_str(seq1hot, idx, msa, pair).to(self.device1)
        xyz_s.append(xyz)
