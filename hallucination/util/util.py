...
def write_pdb_str_sc(seq, atoms, Bfacts=None, pdb_idx=None,h=False):
    L = len(seq)
    idx = np.arange(L)
    ctr = 0
    if Bfacts == None:
    ...
    if pdb_idx is None:
        pdb_idx = [('A', i+1) for i in range(L)]
    wrt = ""
    for i,(ch,i_pdb),s in zip(idx, pdb_idx, seq): #enumerate(seq):
        s=aa2num[aa123[s]]
        atms = aa2long[s]
        if h:
            atms = aa2long_h[s]
        for j,atm_j in enumerate(atms):
            if (atm_j is not None):
                ctr += 1
                wrt += "%-6s%5s %4s %3s %s%4d    %8.3f%8.3f%8.3f%6.2f%6.2f\n"%(
                    "ATOM", ctr, atm_j, num2aa[s], 
