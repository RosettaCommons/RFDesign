...
class MSARowAttentionWithPair(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, n_head=8, d_hidden=32):
        super(MSARowAttentionWithPair, self).__init__()
...
class MSAColGlobalAttention(nn.Module):
    ...
    def forward(self, msa):
        ...
        #
        query = self.to_q(msa).reshape(B, N, L, self.h, self.dim)
        query = query.mean(dim=1) # (B, L, self.h, self.dim)
        key = self.to_k(msa)
        value = self.to_v(msa)
        gate = torch.sigmoid(self.to_g(msa))
        #
        query = query * self.scaling
        attn = einsum('bihd,bkid->bihk', query, key)
        attn = F.softmax(attn, dim=-1)
        #
        out = einsum('bihk,bkid->bihd', attn, value).reshape(B, 1, L, -1)
        out = gate * out
        #
        out = self.to_out(out)
        return out
...
class TriangleMultiplication(nn.Module):
    ...
    def forward(self, pair):
        ...
        if self.outgoing:
            out = einsum('bikd,bjkd->bijd', left, right)
        else:
            out = einsum('bkid,bkjd->bijd', left, right)
