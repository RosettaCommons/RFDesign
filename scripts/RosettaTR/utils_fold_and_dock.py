...
def gen_rst(params, L1):
    ...
    if params['PERMUTE']:
        ...
        omega = np.transpose(omega,(1,2,0))
...
def gen_ambrst(params, nres, nsymm):
    ...
    for a,b,p,p_12 in zip(i,j,prob,prob_12):
        ...
        ambcst = rosetta.core.scoring.constraints.AmbiguousConstraint()
        for i_symm in range(nsymm):
            shift = i_symm * nres
            ida = rosetta.core.id.AtomID(5,a+1)
            idb = rosetta.core.id.AtomID(5,shift+b+1)
            cst = rosetta.core.scoring.constraints.AtomPairConstraint(ida, idb, spline)
            ambcst.add_individual_constraint(cst)
        rst['dist'].append([a,b,p,p_12,ambcst])
    print("ambiguous dist restraints:    %d"%(len(rst['dist'])))
...
def set_random_dihedral(pose, nres):
    #nres = pose.total_residue()
...
def add_rst(pose, rst, sep1, sep2, params, nogly=True, use_orient=None, pcut=None, p12_cut=0.0):
    if use_orient == None:
...
def symmetrize(pose,symmdef_fn):
    pose_symm = pose.clone()
    rosetta.core.pose.symmetry.make_symmetric_pose(pose_symm,symmdef_fn)
    pose.assign(pose_symm)

def asymmetrize( pose_symm, nres_asymm ):
    pose_asymm = Pose()
    rosetta.core.pose.symmetry.extract_asymmetric_unit(pose_symm, pose_asymm, False)
    #pose_asymm = pose_symm.clone()
    #rosetta.core.pose.symmetry.make_asymmetric_pose( pose_asymm )
    #pose_asymm.delete_residue_range_slow( nres_asymm+1, pose_asymm.size() )
    #pose_symm.assign(pose_asymm)
    return pose_asymm

def rigidbody_pert(pose):
...
def dock_low_res(pose, sf_low):
    slide = rosetta.protocols.docking.DockingSlideIntoContact(1)
    slide.apply(pose)
    dock_lowres = rosetta.protocols.docking.DockingLowRes(sf_low, 1)
    dock_lowres.apply(pose)

def dock_high_res(pose, sf_fa):
    slide = rosetta.protocols.docking.FaDockingSlideIntoContact(1)
    slide.apply(pose)
    dock_highres = rosetta.protocols.docking.DockMCMProtocol()
    dock_highres.set_scorefxn(sf_fa)
    dock_highres.set_partners("A_B")
    dock_highres.apply(pose)
